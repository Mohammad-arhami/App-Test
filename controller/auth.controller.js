...
const signUp = async ( req , res , next) => {
    ...
    try {
        ...
        if ( password < 8) {
            return next(errorHandler(401 , "Password must be over than 6 characters"))
        }
    
        const validUser = await User.findOne({email});
    
        if (validUser) {
            return next(errorHandler(401 , "User with given email is alredy exists"))
        }
...
const verify = async (req,res,next) => {
    ...
    try {
        ...
        if (!email || !code ) {
            return next(errorHandler(401 , "The field are required"));
        }

        const findUser = await Verification.findOne({email});

        if (!findUser) {
            return next(errorHandler(401 , "User doesn't exist or the email is wrong"))
        };

        if (findUser.code !== code ) {
            return next(errorHandler(401 , "The code entered is incorrect"));
        }

        if (new Date(findUser.expiresAt) < new Date()) {
